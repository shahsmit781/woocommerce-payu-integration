...
class PayU_Token_Manager {
	...
	const SCOPE_UPDATE_PAYMENT_LINKS = 'update_payment_links';
	const SCOPE_SHARE_PAYMENT_LINKS = 'create_payment_links update_payment_links read_payment_links';
	...
	/**
	 * Get PayU access token for update_payment_links scope (e.g. change status/expiry).
	 *
	 * @param string $merchant_id   PayU Merchant ID.
	 * @param string $client_id     Client ID.
	 * @param string $client_secret Client Secret (plain).
	 ...
	 * @return string|WP_Error Access token on success, WP_Error on failure.
	 */
	public static function get_token_for_update_payment_link( $merchant_id, $client_id, $client_secret, $environment ) {
		$scope   = self::SCOPE_UPDATE_PAYMENT_LINKS;
		$scope   = function_exists( 'payu_normalize_scope_string' ) ? payu_normalize_scope_string( $scope ) : $scope;
		$hash    = function_exists( 'payu_scope_hash' ) ? payu_scope_hash( $scope ) : hash( 'sha256', $scope );
		$env_db  = self::normalize_environment_for_db( $environment );
	...
	/**
	 * Get PayU access token for Share Payment Link API (combined scope: create_payment_links update_payment_links read_payment_links).
	 *
	 * @param string $merchant_id   PayU Merchant ID.
	 * @param string $client_id     Client ID.
	 * @param string $client_secret Client Secret (plain).
	 ...
	 * @return string|WP_Error Access token on success, WP_Error on failure.
	 */
	public static function get_token_for_share_payment_link( $merchant_id, $client_id, $client_secret, $environment ) {
		$scope   = self::SCOPE_SHARE_PAYMENT_LINKS;
		$scope   = function_exists( 'payu_normalize_scope_string' ) ? payu_normalize_scope_string( $scope ) : $scope;
		$hash    = function_exists( 'payu_scope_hash' ) ? payu_scope_hash( $scope ) : hash( 'sha256', $scope );
		$env_db  = self::normalize_environment_for_db( $environment );
